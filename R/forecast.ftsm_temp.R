...
#' @param FUN forecasting function (if provided, \code{method} is ignored)
#' @param FUN_method Method to be used for conformalization (simulation of calibrated residuals)
#' when \code{FUN} is not \code{NULL}.
#' @param FUN_nsim Number of simulations when \code{FUN} is not \code{NULL}.
#' @param FUN_block_size Block size for block-bootstrap when \code{FUN} is not \code{NULL}.
#' @param FUN_seed Seed for reproducibility when \code{FUN} is not \code{NULL}.
...
#' 
#' @export
forecast.ftsm <- function (object, h = 10, 
                           method = c("ets", "arima", "ar", "ets.na",
                                      "rwdrift", "rw", "struct", "arfima", 
                                      "ridge2f"), level = 95, 
                           jumpchoice = c("fit", "actual"), 
                           pimethod = c("parametric", "nonparametric"), 
                           B = 100, usedata = nrow(object$coeff), 
                           adjust = TRUE, 
                           model = NULL, damped = NULL, 
                           stationary = FALSE, FUN = NULL,
                           FUN_method = c("block-bootstrap", "surrogate", 
                           ...
                           ...)
{
  ...
  else if(!is.null(FUN)) # custom, conformalized model 
  {
    FUN_method <- match.arg(FUN_method)
    for(i in 1:nb)
    {
      if(var(xx[,i], na.rm=TRUE) < 1e-8)
      {
      ...
      {
        fit_obj <- ahead::conformalize(FUN=FUN, y=xx[, i], h=h, 
                                       level=level, 
                                       method=FUN_method,
                                       nsim=FUN_nsim,
                                       block_size=FUN_block_size,
                                       seed=FUN_seed,
                                       ...)
  ...
  if (pimethod == "parametric") {
    ...
    if ((method == "ridge2f" || !is.null(FUN)) && "upper" %in% names(fmodels) && "lower" %in% names(fmodels)) {
      ...
      # other methods different from "ridge2f"
      ...
      coeff <- list()
      for (i in 1:nb) {
        coeff[[i]] <- structure(list(mean = ts(meanfcast[, i], start = ytsp[2] + 1/ytsp[3], frequency = ytsp[3]), 
                                     lower = ts(meanfcast[, i] - qconf * sqrt(varfcast[, i]), start = ytsp[2] + 1/ytsp[3], frequency = ytsp[3]), 
                                     upper = ts(meanfcast[, i] + qconf * sqrt(varfcast[, i]), start = ytsp[2] + 1/ytsp[3], frequency = ytsp[3]), 
                                     level = level, x = x[, i], method = method, 
